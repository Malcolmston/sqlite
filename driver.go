...
// Close implements driver.Stmt. The statement holds no resources of its own, so
// this is a no-op that always returns nil.
func (s *driverStmt) Close() error { return nil }

// NumInput implements driver.Stmt. It returns the number of ? placeholder
// parameters the prepared statement expects.
func (s *driverStmt) NumInput() int { return s.nParam }

// Exec implements driver.Stmt. It converts args to engine values and executes
// the prepared (non-query) statement, returning the resulting driver.Result.
...
// Query implements driver.Stmt. It converts args to engine values and executes
// the prepared query, returning the resulting driver.Rows.
...
// Rollback implements driver.Tx. It aborts the current transaction, discarding
// any changes made within it.
...
// LastInsertId implements driver.Result. It returns the row ID generated by the
// most recent insert executed through this result.
func (r driverResult) LastInsertId() (int64, error) { return r.lastID, nil }

// RowsAffected implements driver.Result. It returns the number of rows the
// statement inserted, updated, or deleted.
...
// Columns implements driver.Rows. It returns the names of the columns in the
// result set, in selection order.
func (r *driverRows) Columns() []string { return r.rs.Columns }

// Close implements driver.Rows. The result set is fully materialized in memory,
// so there is nothing to release and it always returns nil.
func (r *driverRows) Close() error { return nil }

// Next implements driver.Rows. It writes the next row's column values into dest
// as their Go representations, padding any missing trailing columns with nil,
// and returns io.EOF once the result set is exhausted.
